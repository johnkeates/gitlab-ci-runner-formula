...
register-runner-{{ name }}:
  cmd.run:
    - name: gitlab-runner register -u {{ multirunner.url }} -r {{ multirunner.token }} --name {{ name }} --executor {{ multirunner.executor }} {% if multirunner.tags %} --tag-list {{ multirunner.tags }} {% endif %} -n
